...
def _render_codex_mcp_section() -> str:
    ...
    environment lacks it. Dev/git installs must instead point Codex at the
    resolved executable; otherwise setup silently downgrades the MCP server
    back to the latest PyPI release and hides main-branch fixes under test.
    """
    ouroboros_executable = shutil.which("ouroboros")
    if ouroboros_executable and _is_dev_ouroboros_build():
        command_lines = "\n".join(
            (
                f"command = {json.dumps(ouroboros_executable)}",
                f"args = {json.dumps(_CODEX_DIRECT_MCP_ARGS)}",
            )
        )
    else:
        command_lines = "\n".join(
